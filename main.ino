...
void gearDrivePushedCallbackFunction(void *s) { bleGamepad.press((char)(15)); }
...
void setup() {
  ...
  bleGamepadConfig.setWhichAxes(enableX, enableY, enableZ, enableRX, enableRY,
                                enableRZ, enableSlider1, enableSlider2);  // 轴
  ...
  bleGamepadConfig.setPid(0x2500);  // 型号（版本号）
...
void loop() {
  ...
  if (bleGamepad.isConnected()) {
    ...
    bleGamepad.setAxes(CurrentAngleAxis, CurrentJoystickXValue,
                       CurrentJoystickYValue, 0, 0, 0, CurrentAcceleratorValue,
                       0);
